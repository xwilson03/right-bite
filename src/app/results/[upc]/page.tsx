...
export default async function Page({params: {upc}}: {params: {upc: string}}) {
    const {data: {links, ingredients}} = await getData(upc);
    ...
    return (
        <div className="p-4 grid gap-6">
            <div className="bg-green-900 p-4 rounded-lg">
                ...
                <p className="text-sm text-zinc-400 mt-2">This product is generally safe based on its ingredients.</p>
            </div>
            <div>
                <h2 className="font-semibold text-lg mb-2 text-white">Ingredients</h2>
                <ul className="space-y-1">
                    {ingredients.map((ingredient, i) => (
                        <li key={i} className="text-sm text-zinc-400">{ingredient}</li>
                    ))}
                </ul>
            </div>
            <div>
                <h2 className="font-semibold text-lg mb-2 text-white">Product Information</h2>
                <p className="text-sm text-zinc-400">
                    This is a brief description of the product. It&apos;s made by a company that has had no recalls or lawsuits.
                </p>
            </div>
        </div>
    )
